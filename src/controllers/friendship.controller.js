...
export const createFriendRequest = asyncHandler(async (req, res, next) => {
  ...
  if (receiverId.toString() === req.user._id?.toString()) {
    return next(new ApiError(BAD_REQUEST, "Haha!! you can't send request to yourself"));
  }
  ...
  if(isFriendshipExists){
    return next(new ApiError(BAD_REQUEST, "Friendship already exists, please refresh the page"));
  }
  ...
  res
    .status(OK)
    .json(new ApiResponse(OK, "Friend request created", {friendshipId: friendship._id}));
});
...
export const respondFriendRequest = asyncHandler(async (req, res, next) => {
  ...
  let friendship = await Friendship.findById(friendshipId).populate("requestSenderId");
  if (!friendship) {
  ...
  if(friendship.status === "accepted"){
    return res
      .status(OK)
      .json(new ApiResponse(OK, "Friend request already accepted", {alreadyFriends:true}));
  }
  if(status === "rejected"){
    await Friendship.findByIdAndDelete(friendshipId);
    return res
      .status(OK)
      .json(new ApiResponse(OK, "Friend request rejected"));
  }
  ...
  res
    .status(OK)
    .json(new ApiResponse(OK, "Friend request responded"));
});
...
export const cancelFriendRequest = asyncHandler(async (req, res, next) => {
  ...
  if(friendship.requestSenderId.toString() !== req.user._id.toString()){
    return next(new ApiError(BAD_REQUEST, "You can only cancel your own friend request"));
  }
  await Friendship.findByIdAndDelete(friendshipId);
  res
    .status(OK)
    .json(new ApiResponse(OK, "Friend request canceled"));
});
...
export const getFriendRequests = asyncHandler(async (req, res, next) => {
  ...
  const totalRequests = friendRequestsResults[0].total[0] ? friendRequestsResults[0].total[0].count : 0;
  const hasMore = (page * limit) < totalRequests;
  res
    .status(OK)
    .json(new ApiResponse(OK, "Get friend request success", {friendRequests, totalRequests, hasMore}));
});
...
export const getAllFriends = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(OK)
    .json(new ApiResponse(OK, "Get all friends success", friends));
});
